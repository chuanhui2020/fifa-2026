...
async function executeAttribution(
  ...
  // Mirror the clamp band the validator actually enforces (validate.ts):
  // a deterministic odds/Elo anchor is trusted tightly; a soft/LLM/uniform
  // base gives the model more room since it isn't a reliable anchor.
  const bandPP = baseProbability.deterministic ? 15 : 35;
  const anchorNote = baseProbability.deterministic
    ? `This base is a reliable anchor (devigged market odds / Elo) and ALREADY prices in public info — recent form, injuries, ratings, home advantage. Do not move it just to restate something it already reflects; only deviate for information it plausibly hasn't priced yet.`
    : `This base is a soft fallback estimate (no deterministic odds/Elo available), so you have more room to adjust based on the collected factors. Stay sane and explain your reasoning.`;
  const baseInfo = isKnockout
    ? `\nBase Probability (from ${baseProbability.source} data, knockout stage — no draw):\n- Home advances: ${(baseProbability.homeWin * 100).toFixed(1)}%\n- Away advances: ${(baseProbability.awayWin * 100).toFixed(1)}%\n\n${anchorNote}\nThis is a knockout match — extra time and penalties decide the winner if needed. Set draw to exactly 0. You may adjust each outcome by at most ±${bandPP} percentage points. Deviations beyond this band will be clamped.\n`
    : `\nBase Probability (from ${baseProbability.source} data):\n- Home win: ${(baseProbability.homeWin * 100).toFixed(1)}%\n- Draw: ${(baseProbability.draw * 100).toFixed(1)}%\n- Away win: ${(baseProbability.awayWin * 100).toFixed(1)}%\n\n${anchorNote}\nUse this as your starting point. You may adjust each outcome by at most ±${bandPP} percentage points (e.g. a 40% base allows ${40 - bandPP}%–${40 + bandPP}%). Deviations beyond this band will be clamped, so keep within it and explain any significant adjustment.\n`;
